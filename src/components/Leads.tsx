...
import { getCompanies, getOpportunities, saveCompany, getCurrentUser } from '../store';
...
const Leads: React.FC<LeadsProps> = ({ onNavigateToPipeline }) => {
  ...
  const [newActivity, setNewActivity] = useState<Partial<StoreActivity>>({
    type: 'Telefone', notes: '', date: getLocalDatetime()
  });

  useEffect(() => {
    Promise.all([getCompanies(), getOpportunities()])
      .then(([comps, opps]) => {
        setCompanies(comps);
  ...
  const getOpenOpportunities = (companyId: string) =>
    opportunities.filter(o => o.companyId === companyId && !o.isLost && o.status !== 'Fechamento');
  ...
  const handleSave = async () => {
    if (!formData.fantasyName) { alert('Por favor, preencha o Nome Fantasia.'); return; }
    setSaving(true);
  ...
  const handleAddActivity = async () => {
    if (!newActivity.notes) { alert('Por favor, preencha as notas do atendimento.'); return; }
    ...
    if (formData.id) {
      const updatedCompany = { ...formData, contacts, activities: updatedActivities } as Company;
      await saveCompany(updatedCompany);
  ...
  const updateActivity = async (id: string, field: keyof StoreActivity, value: string) => {
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: '2rem', alignItems: 'flex-start' }}>
            ...
            <div style={{ gridColumn: 'span 6', backgroundColor: 'var(--surface-color)', padding: '1.5rem', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border-color)' }}>
              ...
              <div className="form-group">
                ...
                <input value={formData.corporateName || ''} onChange={e => setFormData({ ...formData, corporateName: e.target.value })} />
              </div>
              ...
              <div style={{ padding: '1.25rem', border: '1px solid var(--border-color)', borderRadius: '0.5rem', marginBottom: '1.5rem', backgroundColor: '#F8FAFC', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                ...
                <div className="flex gap-4">
                  ...
                  <div className="form-group flex-1">
                    ...
                    <input type="datetime-local" value={newActivity.date} onChange={e => setNewActivity({ ...newActivity, date: e.target.value })} />
                  </div>
                </div>
                <div className="form-group">
                ...
                {activities.map(activity => {
                  ...
                  return (
                    <div key={activity.id} style={{ backgroundColor: colors.bg, border: `1px solid ${colors.border}`, padding: '1.25rem 1.5rem', marginBottom: '1.25rem', borderRadius: '0.75rem', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
                        ...
                        {isMasterUser && (
                          <div style={{ display: 'flex', gap: '0.25rem', alignItems: 'center' }}>
                            <button
                              ...
                              style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', padding: '0.25rem 0.5rem', fontSize: '0.7rem', fontWeight: 600, color: editingActivities[activity.id] ? '#047857' : '#64748B', backgroundColor: editingActivities[activity.id] ? '#D1FAE5' : 'rgba(255,255,255,0.5)', borderRadius: '1rem', border: `1px solid ${editingActivities[activity.id] ? '#A7F3D0' : 'transparent'}`, cursor: 'pointer' }}
                            >
                              <Edit2 size={10} /> {editingActivities[activity.id] ? 'Concluir' : 'Editar'}
                            </button>
                            <button
                              onClick={() => deleteActivity(activity.id)}
                              style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0.35rem', color: '#EF4444', backgroundColor: 'transparent', borderRadius: '50%', border: 'none', cursor: 'pointer', opacity: 0.7 }}
                            >
                              <Trash2 size={12} />
                            </button>
                          </div>
                      ...
                      {isMasterUser && editingActivities[activity.id] ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                          <div style={{ display: 'flex', gap: '1rem' }}>
                            <select value={activity.type} onChange={e => updateActivity(activity.id, 'type', e.target.value)} style={{ flex: 1, padding: '0.25rem', fontSize: '0.875rem', borderRadius: '0.25rem', border: `1px solid ${colors.border}`, backgroundColor: 'rgba(255,255,255,0.7)', outline: 'none' }}>
                              <option value="Telefone">Telefone</option>
                              <option value="WhatsApp">WhatsApp</option>
                              <option value="E-mail">E-mail</option>
                              <option value="Visita">Visita</option>
                          ...
                          <textarea value={activity.notes} onChange={e => updateActivity(activity.id, 'notes', e.target.value)} rows={3} style={{ width: '100%', padding: '0.5rem', borderRadius: '0.25rem', border: `1px solid ${colors.border}`, backgroundColor: 'rgba(255,255,255,0.7)', color: 'var(--text-main)', outline: 'none', fontSize: '0.875rem', resize: 'vertical' }} />
                        </div>
                      ) : (
                        <div style={{ fontSize: '0.875rem', color: 'var(--text-main)', whiteSpace: 'pre-wrap', lineHeight: 1.5 }}>{activity.notes}</div>
                      )}
                    </div>
                  );
                })}
  ...
  return (
    <>
      <header className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <button className="btn btn-primary" onClick={() => openEdit()}>Novo Lead</button>
      </header>

      <div className="page-body">
        {loading ? (
          <div className="p-8 text-center text-muted">Carregando leads...</div>
        ) : (
            ...
            {companies.map(company => {
              ...
              return (
                <div
                  ...
                  style={{ gridColumn: 'span 12', cursor: 'pointer', transform: 'none', display: 'flex', flexDirection: 'column', gap: '1.5rem', padding: '1.5rem' }}
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '2rem', alignItems: 'flex-start' }}>
                    <div style={{ flex: '1 1 350px', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1.25rem' }}>
                        {company.segment && <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', fontSize: '0.875rem', color: 'var(--text-muted)' }}><Briefcase size={16} /> {company.segment}</div>}
                        {company.website && (
                          <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', fontSize: '0.875rem' }}>
                            <Globe size={16} color="var(--text-muted)" />
                            <a href={company.website.startsWith('http') ? company.website : `https://${company.website}`} target="_blank" rel="noreferrer" style={{ color: 'var(--primary-color)', textDecoration: 'none' }} onClick={e => e.stopPropagation()}>
                              {company.website.replace(/^https?:\/\//, '')}
                            </a>
                          </div>
                        )}
                        {company.phone && <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', fontSize: '0.875rem' }}><Phone size={16} color="var(--text-muted)" /><span>{company.phone}</span></div>}
                      ...
                      <div style={{ marginTop: '0.5rem', padding: '1rem', borderRadius: 'var(--radius-md)', backgroundColor: openOpps.length > 0 ? '#FEFCE8' : '#F9FAFB', border: `1px solid ${openOpps.length > 0 ? '#FEF08A' : 'var(--border-color)'}` }}>
                        ...
                        {openOpps.length > 0 && (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', marginLeft: '1.5rem' }}>
                            {openOpps.map(o => (
                              <div key={o.id} style={{ fontSize: '0.8rem', color: 'var(--text-muted)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.25rem' }} onClick={e => { e.stopPropagation(); setSelectedOppForSummary(o); }}>
                                • <strong style={{ color: 'var(--primary-color)' }} className="hover:underline">{o.name}</strong> - {o.status} ({o.responsible})
                              </div>
                            ))}
                          </div>
                        )}
                    ...
                    <div style={{ flex: '1 1 350px' }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '0.75rem' }}>
                        ...
                        {company.contacts.length === 0 && <p style={{ fontSize: '0.875rem', color: 'var(--text-muted)' }}>Nenhum contato cadastrado.</p>}
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div className="modal-content" style={{ maxWidth: '450px' }} onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <h3
                ...
                onClick={() => { setSelectedOppForSummary(null); if (onNavigateToPipeline) onNavigateToPipeline(selectedOppForSummary.id); }}
              >
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {[
                  { label: 'Status Atual', value: selectedOppForSummary.status },
                  { label: 'Valor Estimado', value: new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(selectedOppForSummary.estimatedValue), color: 'var(--success-color)' },
                  { label: 'Responsável', value: selectedOppForSummary.responsible },
                  selectedOppForSummary.expectedClosingDate ? { label: 'Previsão de Fechamento', value: new Date(selectedOppForSummary.expectedClosingDate).toLocaleDateString('pt-BR', { timeZone: 'UTC' }) } : null,
                  selectedOppForSummary.nextFollowUp ? { label: 'Próximo Follow-up', value: new Date(selectedOppForSummary.nextFollowUp).toLocaleDateString('pt-BR'), color: '#4338CA' } : null,
                ].filter(Boolean).map((item: any) => (
                  <div key={item.label} style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: '0.5rem', borderBottom: '1px solid var(--border-color)' }}>
