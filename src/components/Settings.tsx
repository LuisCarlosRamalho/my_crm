...
const Settings: React.FC<SettingsProps> = ({ onProfileUpdate }) => {
  ...
  const handleSavePassword = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    const updatedUser = { ...currentUser, passwordHash: CryptoJS.SHA256(newPassword).toString() };
    const updatedList = users.map(u => u.id === updatedUser.id ? updatedUser : u);
    await saveUsers(updatedList);
    setCurrentUser(updatedUser);
    setLocalCurrentUser(updatedUser);
  ...
  const handleDeleteUser = async (id: string) => {
    if (id === currentUser?.id) { alert('Você não pode excluir sua própria conta aqui.'); return; }
  ...
  const handleSaveAdminPassword = async () => {
    ...
    if (passError) { alert(passError); return; }
    const updatedUser = { ...editingUser, passwordHash: CryptoJS.SHA256(adminNewPassword).toString() };
    const updatedList = users.map(u => u.id === updatedUser.id ? updatedUser : u);
    await saveUsers(updatedList);
    setUsers(updatedList);
    setEditingUser(null);
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: '2rem', alignItems: 'flex-start' }}>
            ...
            <div style={{ gridColumn: 'span 6', backgroundColor: 'var(--surface-color)', padding: '1.5rem', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border-color)' }}>
              ...
              <form onSubmit={handleSavePassword} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                ...
                <button type="submit" className="btn btn-secondary mt-2" disabled={saving}>
                  ...
                  {users.map(u => (
                    <div key={u.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem', backgroundColor: '#F8FAFC', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-md)' }}>
                      ...
                      <div style={{ display: 'flex', gap: '0.5rem' }}>
                        <button className="btn btn-secondary" style={{ padding: '0.35rem 0.5rem', fontSize: '0.75rem' }} onClick={() => setEditingUser(u)} title="Alterar Senha">
                          <Edit2 size={14} /> Senha
                        </button>
                        {u.id !== currentUser.id && (
                          <button className="btn btn-danger" style={{ padding: '0.35rem 0.5rem', fontSize: '0.75rem' }} onClick={() => handleDeleteUser(u.id)} title="Excluir Usuário">
                            <Trash2 size={14} />
                          </button>
                        )}
                      </div>
